...
async function writeToD1(env, body, slug, ring) {
  try {
    ...
    for (const e of (body.entries || [])) {
      ...
      // ── JUMPER results ────────────────────────────────────────────────────────
      // Field names from watcher (confirmed 2026-03-22):
      //   r1TotalTime, r1TimeFaults, r1JumpFaults, r1TotalFaults, overallPlace, statusCode
      //   r2TotalTime, r2TimeFaults, r2JumpFaults, r2TotalFaults
      //   r3TotalTime, r3TimeFaults, r3JumpFaults, r3TotalFaults

      if (isJumper && e.r1TotalTime) {
        await env.WEST_DB.prepare(`
          INSERT INTO results (entry_id, class_id, round, time, jump_faults, time_faults, total, place, status_code, created_at, updated_at)
          VALUES (?, ?, 1, ?, ?, ?, ?, ?, ?, ?, ?)
